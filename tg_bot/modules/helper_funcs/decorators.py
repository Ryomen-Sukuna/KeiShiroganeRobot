            ...
            try:
                if can_disable:
                    ...
                    self._dispatcher.add_handler(
                        ...
                        group)
                log.info(
                    f"[KEICMD] Loaded handler {command} for function {func.__name__} in group {group}")
            except TypeError:
                if can_disable:
                    self._dispatcher.add_handler(
                        DisableAbleCommandHandler(
                    ...
                    self._dispatcher.add_handler(
                        CommandHandler(
                            ...
                            pass_chat_data=pass_chat_data))
                log.info(
                    f"[KEICMD] Loaded handler {command} for function {func.__name__}")
            ...
            try:
                if can_disable:
                    ...
                    self._dispatcher.add_handler(MessageHandler(
                        pattern, func, run_async=run_async), group)
                log.info(
                    f"[KEIMSG] Loaded filter pattern {pattern} for function {func.__name__} in group {group}")
            except TypeError:
                if can_disable:
                    self._dispatcher.add_handler(DisableAbleMessageHandler(
                ...
                log.info(
                    f"[KEIMSG] Loaded filter pattern {pattern} for function {func.__name__}")
        ...
        def _callbackquery(func):
            self._dispatcher.add_handler(
                CallbackQueryHandler(
                    ...
                    run_async=run_async))
            log.info(
                f'[KEICALLBACK] Loaded callbackquery handler with pattern {pattern} for function {func.__name__}')
            return func
...
keicallback = KeiTelegramHandler(d).callbackquery
keiinline = KeiTelegramHandler(d).inlinequery
